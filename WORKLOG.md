...
- Doctor directory: user-এর `supabase/doctors_source.json` (189 জন ডায়াবেটিস/হরমোন বিশেষজ্ঞ, 22 hospital)। এরা app user নন → আলাদা `doctor_directory` table (`migration_003_doctor_directory.sql`, schema.sql-এও merge; public read, admin write; `admin_stats`-এ directory count)। `build_doctor_seed.mjs` → `seed_doctors.sql` (upsert; hospital নাম alias করে `hospital_id` link)। `/doctors` page নতুন: search + জেলা filter, উপরে registered (app-এ appointment), নিচে directory card (degree, হাসপাতাল, call button), pagination 30। Admin: `/admin/directory` (list/search/filter, `/new`, `/[id]` edit/delete), nav + stat tile
- User Supabase-এ migration 002, 003, seed_hospitals, seed_doctors সব চালিয়েছে ✔ (DB যাচাই: 136 hospital, 189 doctor, 184 linked)। `/doctors` live screenshot OK; hospital নামে জেলা থাকলে দ্বিগুণ দেখানো fix
- Hospital ↔ doctor linking (user request): নতুন `/hospitals/[id]` (তথ্য, call, direction, mini map, ওই hospital-এর সব directory doctor + registered doctor) ও `/directory/[id]` (doctor detail: degree, hospital link, call button, একই hospital-এর অন্য ডাক্তার)। `/doctors` search-এ hospital-ও মেলে ("হাসপাতাল" section → hospital page)। `components/doctor-card.tsx` shared card (নাম → doctor page, hospital → hospital page)। `/hospitals` list-এর নাম ও map popup-এ "ডাক্তার দেখুন →" link। Mobile-এ grid `grid-cols-1` (implicit auto track overflow fix)। `/directory` proxy-protected
