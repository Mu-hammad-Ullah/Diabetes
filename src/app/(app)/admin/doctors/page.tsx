          ...
          {doctors.map((d) => (
            <li key={d.id} className="card flex flex-wrap items-center justify-between gap-3">
              <div className="min-w-0 text-sm">
                <div className="font-semibold"><Link href={`/admin/users/${d.profile_id}`} className="hover:text-teal-700">{d.profiles?.full_name || '—'}</Link> <span className={`badge ml-2 ${d.is_verified ? 'bg-emerald-100 text-emerald-800 ring-emerald-200' : 'bg-amber-100 text-amber-800 ring-amber-200'}`}>{d.is_verified ? t('verified') : t('notVerified')}</span></div>
